...
`gatefs` is designed for sandboxes that need filesystem composition as free as `mount --bind`, with runtime authorization and background audit trails rooted in the design. It is an experimental, observable filesystem protection shim built on [`fuser`](https://github.com/cberner/fuser). It gives a process a FUSE-backed filesystem view whose read, write, and metadata permissions can be inspected, protected, granted, denied, and adjusted at runtime.
...
```sh
cargo install --git https://github.com/xz-dev/gatefs.git gatefs
```
...
Ctrl-C in the `gatefs run demo` terminal also stops the session.
